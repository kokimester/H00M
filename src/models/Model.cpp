...
bool Model::LoadMesh(const std::filesystem::path& path)
{
    ...
    if (m_Scene) {
        m_GlobalInverseTransform = convertAssimpMatToGLM(m_Scene->mRootNode->mTransformation);
...
void Model::CountVerticesAndIndices(unsigned int& NumVertices, unsigned int& NumIndices)
{
...
void Model::InitSingleMesh(size_t MeshIndex, const aiMesh* paiMesh)
{
    ...
    for (unsigned int i = 0 ; i < paiMesh->mNumVertices ; i++) {
        ...
        if (paiMesh->mNormals) {
            ...
            //TODO: for some reason normals are backwards with this import strategy, so we have to flip it here
...
void Model::LoadSingleBone(size_t MeshIndex, const aiBone* pBone)
{
    ...
    for (size_t i = 0 ; i < pBone->mNumWeights ; i++) {
        ...
        size_t GlobalVertexID = m_Meshes[MeshIndex].BaseVertex + pBone->mWeights[i].mVertexId;
...
size_t Model::FindPosition(float AnimationTimeTicks, const aiNodeAnim* pNodeAnim)
{
...
void Model::CalcInterpolatedPosition(aiVector3D& Out, float AnimationTimeTicks, const aiNodeAnim* pNodeAnim)
{
...
size_t Model::FindRotation(float AnimationTimeTicks, const aiNodeAnim* pNodeAnim)
{
...
size_t Model::FindScaling(float AnimationTimeTicks, const aiNodeAnim* pNodeAnim)
{
...
void Model::ReadNodeHierarchy(float AnimationTimeTicks, const aiNode* pNode, const glm::mat4& ParentTransform)
{
    ...
    if (pNodeAnim) {
        ...
        //std::println("[{}] scaling: {} {} {}",NodeName, Scaling.x, Scaling.y, Scaling.z);
        ...
        //std::println("[{}] quaternion: {} {} {} {}",NodeName, RotationQ.x, RotationQ.y, RotationQ.z, RotationQ.w);
        //RotationM = convertAssimpMatToGLM(RotationQ.GetMatrix());
        ...
        //std::println("[{}] translation: {} {} {}",NodeName, Translation.x, Translation.y, Translation.z);
...
void Model::GetBoneTransforms(float TimeInSeconds, std::vector<glm::mat4>& Transforms)
{
    ...
    float TicksPerSecond = (float)(m_Scene->mAnimations[m_ActiveAnimation]->mTicksPerSecond != 0 ? m_Scene->mAnimations[m_ActiveAnimation]->mTicksPerSecond : 25.0f);
    float TimeInTicks = TimeInSeconds * TicksPerSecond;
    float AnimationTimeTicks = fmod(TimeInTicks, (float)m_Scene->mAnimations[m_ActiveAnimation]->mDuration);
...
void Model::LoadTextures(const std::string& Dir, const aiMaterial* pMaterial, int index)
{
