...
int main()
{
	Window window = Window(1366, 768, GL_FALSE);
	window.Initialise();
	...
	//create camera
	Camera camera(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), -90.0f, 0.0f, 2.0f, 0.15f);
	...
	//compile shaders
	Shader shader("vertex_source.glsl", "fragment_source.glsl");

	glm::mat4 projection(1.f);
	projection = glm::perspective(45.0f, (GLfloat)(window.getBufferWidth() / window.getBufferHeight()), 0.1f, 100.0f);
	...
	while (!window.getShouldClose()) //returns true if window is closed
	{
		...
		{
			spotLights[0].update(camera.getCameraPosition(), camera.getCameraFront(), camera.getRight());
		}
		else {
		...
		//model = glm::rotate(model, glm::radians(triRotation), glm::vec3(0.0f, 1.0f, 0.0f));
		...
		shader.useMaterial(shinyMaterial, "material.shininess", "material.specularIntensity");
		...
		shader.useMaterial(dullMaterial, "material.shininess", "material.specularIntensity");
