...
std::string Shader::loadShaderSource(const char* fileName)
{
	...
	{
		std::cerr << "ERROR WHILE OPENING SHADER SOURCODE: " << fileName << std::endl;
	}
	inFile.close();
...
GLuint Shader::addShader(const char* fileName, GLenum shaderType) //GLuint program
{
	...
	glGetShaderiv(theShader, GL_COMPILE_STATUS, &result); //ellenorizzuk sikerult e
	if (!result)
	{
...
void Shader::setDirectionalLight(DirectionalLight& toSet)
{
	...
	//toSet.useLight(uniformDirectionalLight.uniformAmbientIntensity, uniformDirectionalLight.uniformColour,
	//	uniformDirectionalLight.uniformDiffuseIntensity, uniformDirectionalLight.uniformDirection);
}

void Shader::useLight(const DirectionalLight& toUse, const GLchar* colourName, const GLchar* ambientName,
...
void Shader::useMaterial(const Material& toUse, const GLchar* shininessName, const GLchar* specularName)
{
	glUniform1f(glGetUniformLocation(id, shininessName), toUse.getShininess());
	glUniform1f(glGetUniformLocation(id, specularName), toUse.getSpecularIntensity());
